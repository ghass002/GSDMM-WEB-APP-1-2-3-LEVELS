...
def get_model_results_gsdmm_sub(length, indice, df, n_of_words,texts,mgp=None):
    """funtion to extract model result of LEVEL 2 or LEVEL 3 such as topics, keywords and insert in a pandas dataframe
    in: df : pd.DataFrame() that contains the data input text
    in: n_of_words : number of topics keywords requested
    in: mgp: the trained model
    in: indice: which represents the level; 2 if level 2 and 3 if level 3
    in: length: represents the number of SUB LEVEL
    out: dataframe and dictionary that contains topics and keywords
    """
    ...
    #Extracting and showing number of documents per topic
    doc_count = np.array(mgp.cluster_doc_count)
    if indice == 2:
        st.write('Number of documents per topic (LEVEL 2) SUB LEVEL' + ' ' + str(length+1)+ ':', doc_count)
    else:
        st.write('Number of documents per topic (LEVEL 3) SUB LEVEL' + ' ' + str(length+1)+ ':', doc_count)

    # Topics sorted by the number of document they are allocated to
    top_index = doc_count.argsort()[-30:][::-1]
    
    # Showing document frequencies dictionary
    if indice == 2:
        st.success('Dictionary for doc frequency (LEVEL 2)')
    else:
        st.success('Dictionary for doc frequency (LEVEL 3)')
    ...
    #if indice == 2:
    #    st.success('Dictionary for top keywords (LEVEL 2)')
    #else:
    #    st.success('Dictionary for top keywords (LEVEL 3)')
    ...
    df.insert(loc=1, column='no_of_docs_per_topic', value=doc_number_list)
    df.insert(loc=2, column='topic_keywords', value=words_list)
...
def generate_labels_de_sub(length, indice, dictionary, topics_df, n_of_topics):
    """funtion to generate labels for each document list LEVEL 2 or 3 
    in: dictionary : dictionary that contains topic keywords
    in: n_of_topics : number of topics requested
    in: topics_df: dataframe that contains text and the topic number of each text
    in: indice: which represents the level; 2 if level 2 and 3 if level 3
    in: length: represents the number of SUB LEVEL
    out: list of labels and document list of each topic for LEVEL 3
    """
    ...
    if indice == 2:       
        st.success('Generated Labels (LEVEL 2)' + ' ' + str(length+1) +':')   
    else:
        st.success('Generated Labels (LEVEL 3)' + ' ' + str(length+1) +':') 
    st.write(labels_count)
    ...
    topics_df.insert(loc=4, column='topic_label', value=topic_label)
...
def add_cells(indice, df, topics_df):
    """funtion to extract the cells from dataframe of LEVEL 2 or LEVEL 3 such as keywords, labels and insert in the downloadable final dataframe
    in: df : pd.DataFrame() that contains the data input text from LEVEL 2 and LEVEL 3
    in: topics_df: the main dataframe to be downloaded    
    in: indice: which represents the level; 2 if level 2 and 3 if level 3    
    out: FINAL DOWNLOADABLE DATAFRAME
    """

    for i in range(len(topics_df['Text'])):

        for j in range(len(df['Text'])):

            if topics_df['Text'][i] == df['Text'][j] and indice == 2:                
                topics_df['no_of_docs_per_topic (LEVEL 2)'][i] = df['no_of_docs_per_topic'][j]
                topics_df['topic_keywords (LEVEL 2)'][i] = df['topic_keywords'][j]
                topics_df['topic_label (LEVEL 2)'][i] = df['topic_label'][j]
                
            if topics_df['Text'][i] == df['Text'][j] and indice == 3:
                topics_df['no_of_docs_per_topic (LEVEL 3)'][i] = df['no_of_docs_per_topic'][j]
                topics_df['topic_keywords (LEVEL 3)'][i] = df['topic_keywords'][j]
                topics_df['topic_label (LEVEL 3)'][i] = df['topic_label'][j]
