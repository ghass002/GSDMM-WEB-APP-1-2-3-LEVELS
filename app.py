...
n_of_topics_2 = st.sidebar.number_input('Expected Number of Topics Level 2',min_value=1,max_value=2000,value=2,step =1)
n_of_topics_3 = st.sidebar.number_input('Expected Number of Topics Level 3',min_value=1,max_value=2000,value=2,step =1)
n_of_docs = st.sidebar.number_input('Maximum number of documents',min_value=1,max_value=10000,value=2,step =1)
...
if ss.to_evaluate:
    ...
    if button_eva:               
        ...
        #Insert Columns for LEVEL 2 and LEVEL 3 TOPIC CLUSTERING into the final dataframe
        ss.topics_df.insert(loc=7, column='no_of_docs_per_topic (LEVEL 2)', value=[0 for i in range(len(ss.topics_df))])
        ss.topics_df.insert(loc=8, column='topic_keywords (LEVEL 2)', value=['' for i in range(len(ss.topics_df))])
        ss.topics_df.insert(loc=9, column='topic_label (LEVEL 2)', value=['' for i in range(len(ss.topics_df))])
        ss.topics_df.insert(loc=10, column='no_of_docs_per_topic (LEVEL 3)', value=[0 for i in range(len(ss.topics_df))])
        ss.topics_df.insert(loc=11, column='topic_keywords (LEVEL 3)', value=['' for i in range(len(ss.topics_df))])
        ss.topics_df.insert(loc=12, column='topic_label (LEVEL 3)', value=['' for i in range(len(ss.topics_df))])     

        #LEVEL 2 and 3 of topic modelling
        for i in range(len(topics_document_list)):
            
            # LEVEL 2
            if len(topics_document_list[i]) >= n_of_docs:
                level = 2 
                ss.clean_text, df_level2 =pp.extract_features_de_sub(topics_document_list[i])
                ss._model = gsdm.gsdmm_train(ss.clean_text,alpha,beta,iterations,number_of_topics=n_of_topics_2)
                df_level2, dictionary = mv.get_model_results_gsdmm_sub(i, level, df_level2, n_of_words,texts = ss.clean_text,mgp=ss._model)
                labels_count, topics_document_list_sub = mv.generate_labels_de_sub(i, level, dictionary,df_level2, n_of_topics_2)
                ss.topics_df = mv.add_cells(level, df_level2,ss.topics_df)
                
                #LEVEL 3 
                for k in range(len(topics_document_list_sub)):

                    if len(topics_document_list_sub[k]) >= n_of_docs:
                        level = 3
                        ss.clean_text, df_level3 =pp.extract_features_de_sub(topics_document_list_sub[k])
                        ss._model = gsdm.gsdmm_train(ss.clean_text,alpha,beta,iterations,number_of_topics=n_of_topics_3)
                        df_level3, dictionary = mv.get_model_results_gsdmm_sub(k, level, df_level3, n_of_words,texts = ss.clean_text,mgp=ss._model)
                        labels_count, topics_document_list_sub3 = mv.generate_labels_de_sub(i, level, dictionary,df_level3, n_of_topics_3)
                        ss.topics_df = mv.add_cells(level, df_level3,ss.topics_df)      
